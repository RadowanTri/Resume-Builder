import { FilePenIcon, PencilIcon, PlusIcon, TrashIcon, Upload, XIcon, UploadCloud } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const [editResumeId, setEditResumeId] = useState(''); // ✅ fixed: consistent name
    ...
    const editTitle = async (event) => {
        event.preventDefault();
        // await updateResume(editResumeId, { title });
        setEditResumeId(''); // ✅ close modal after submit
        setTitle('');
    };
    ...
    const colors = ["#9333ea", "#d97706", "#dc2626"];
    ...
    return (
        <div className='max-w-7xl mx-auto px-4 py-8'>
            ...
            <div className='flex gap-4'>
                ...
                >
                    <Upload className='size-11 p-2.5 bg-gradient-to-r from-purple-500 to-purple-700 text-white rounded-full' />
                    <p className='text-sm group-hover:text-purple-500 transition-all'>Upload Existing</p>
                </button>
            </div>
                ...
                {allResumes.map((resume, index) => {
                    ...
                    return (
                        <button
                            ...
                            style={{
                                background: `linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`, // ✅ fixed: was missing comma
                        ...
                        >
                            ...
                            <div onClick={e => e.stopPropagation()} className="absolute top-1 right-1 group-hover:flex items-center hidden">
                                <TrashIcon onClick={()=>deleteResume(resume._id)} className='size-7 p-1.5 hover:bg-white/50 rounded transition-colors' />
                                <PencilIcon
                                    onClick={() => {
            ...
            {showUploadResume && (
                ...
                >
                    <div onClick={e => e.stopPropagation()} className="relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6">
                        ...
                        <input
                            onChange={(e) => setTitle(e.target.value)} // ✅ fixed: was missing onChange/value
                            value={title}
                            placeholder="Enter resume title"
                            className="w-full px-4 py-2 mb-4 border rounded focus:outline-none focus:ring-2 focus:ring-green-600"
                            type="text" required
                        ...
                        <div>
                            ...
                            <input // ✅ fixed: was missing id and onChange
                                id="resume-input"
                                type="file"
                                accept=".pdf,.doc,.docx"
                                className="hidden"
            ...
            {editResumeId && (
                ...
                >
                    <div onClick={e => e.stopPropagation()} className="relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6">
                        ...
                        <XIcon
                            className='absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colors'
                            onClick={() => { setEditResumeId(''); setTitle(''); }} // ✅ fixed: was setShowEditResume
                        />
                    </div>
                </form>
            )}
